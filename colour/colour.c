...
void Init_ANSIVT_CTABLE( _ANSI_* ANSIobj ){
	...
	char* * ANSIVT_CTABLE = ANSI->ANSIVT_CTABLE;

	#ifdef sys64
	uint64_t i=0;
	uint64_t offset = sizeof( char* );
	#else
	uint32_t i=0;
	...
	*(ANSIVT_CTABLE + i + 4) = NORMAL;
	...
	ANSIobj->ANSIVT_CTABLE = ANSIVT_CTABLE;
	ANSIobj->fg( "white" );
	ANSIobj->bg( "black" );
...
_ANSI_* colorMode()	{
	...
	//ANSI = (struct _ANSI_*) malloc(sizeof(struct _ANSI_)) ;
...
char* getVTCodeString( char* cc )	{
	...
	for( i=0; i <33; i++ )	{
		...
		if( !strcmp( *(ANSI->ANSIVT_CTABLE)+(i*sizeof(char*)*2), cc)  )
			return *(ANSI->ANSIVT_CTABLE)+(i*sizeof(char*)*2) + sizeof(char*);
	}
